...
import com.jxm.mmseg4j.rule.LargestAvgLenRule;
import com.jxm.mmseg4j.rule.LargestSumDegreeFreedomRule;
import com.jxm.mmseg4j.rule.MaxMatchRule;
import com.jxm.mmseg4j.rule.Rule;
import com.jxm.mmseg4j.rule.SmallestVarianceRule;
...
public class ComplexSeg extends Seg {
	...
	private Chunk createChunk(Sentence sen, char[] chs, int[] tailLen,
			int[] offsets, CharNode[] cns/* , char[][] cks */) {
		Chunk ck = new Chunk();

		for (int i = 0; i < 3; i++) {

			if (offsets[i] < chs.length) {
				ck.words[i] = new Word(chs, sen.getStartOffset(), offsets[i],
						tailLen[i] + 1);// new Word(cks[i],
