...
public class CharNode {
	...
	private int freq = -1; // Degree of Morphemic Freedom of One-Character,
							// 单字才需要
	 ...
	 * @param sen
	 *            句子, 一串文本.
	 * @param wordTailOffset
	 *            词在句子中的位置, 实际是 offset 后面的开始找.
	 * @return 返回词尾长, 没有就是 0
	...
	public static class KeyTree {
		...
		public ArrayList<Integer> maxMatch(ArrayList<Integer> tailLens,
				char[] sen, int offset) {
			TreeNode node = head;
			for (int i = offset; i < sen.length; i++) {
				node = node.subNode(sen[i]);
