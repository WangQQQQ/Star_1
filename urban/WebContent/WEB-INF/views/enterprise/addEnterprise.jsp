...
$(function(){
	 ...
	 $("#eName").click(function() {
	  ...
	  $("#eAddress").click(function() {
		  $("#nullError").html("");
		  $("#addressError").html("");
		  $("#addressSuccess").html("");
		  
	  });
	  $("#bidNum").click(function() {
		  $("#nullError").html("");
	  });
	  $("#dt").click(function() {
		  $("#nullError").html("");
	...
	$("#eName").change(function(){
		...
		$.post(url, args, function(data){
			if(data == "0"){
				$("#nameSuccess").html("企业名称可用");
			}else if(data == "1"){
				$("#nameError").html("企业名称已存在，不可用!");
				//alert("企业名称已存在，不可用!");
			}else{
				alert("网络或程序出错. ");
			}
	...
	$("#eAddress").change(function(){
		...
		var url = "${pageContext.request.contextPath }/ajaxValidateAddress";
		var args = {"eAddress":val,"date":new Date()};
		$.post(url, args, function(data){
			if(data == "0"){
				$("#addressSuccess").html("企业地址可用");
				
			}else if(data == "1"){
				$("#addressError").html("企业地址已存在，请重新确认");
				//alert("企业地址已存在，请重新确认");
			}else{
				alert("网络或程序出错. ");
			}
	...
	<div class="navbar navbar-inverse">
		<div class="navbar-inner">
			...
			<ul class="nav pull-right">
				<li class="settings hidden-phone"><a href="" role="button">
						<i class="icon-cog"></i>
				</a></li>
				<li class="dropdown"><a href="#"
					class="dropdown-toggle hidden-phone" data-toggle="dropdown"> 欢迎
						${sessionScope.currentUser } <b class="caret"></b>
				</a>
		...
		<ul id="dashboard-menu">
            ...
            <li class="active">
                ...
                <ul class="active submenu">
                    ...
                    <li><a href="addEnterprise" class="active">企业信息添加</a></li>
                </ul>
            </li>
			<li><a href="geographyList"> <i class="icon-code-fork"></i> <span>地理信息管理</span>
			</a></li>
			<li><a class="dropdown-toggle" href="#"> <i
					class="icon-th-large"></i> <span>城建信息管理</span> <i
			...
			<li><a class="dropdown-toggle" href="#"> <i
					class="icon-calendar-empty"></i> <span>综合数据分析</span> <i
					class="icon-chevron-down"></i>
			</a>
				<ul class="submenu">
			...
			<div id="pad-wrapper" class="new-user">
				...
				<div class="row-fluid form-wrapper">
					...
					<div class="span9 with-sidebar">
						<div class="container">
							<form:form
								action="${pageContext.request.contextPath}/saveEnterprise"
								method="POST" modelAttribute="newEnterprise"
								class="new_user_form inline-input" onsubmit="return check()">
								<%-- <div class="span12 field-box">
									<label>企业编号:</label>
									<form:input class="span9" placeholder="企业编号" path="eId"
										id="eId" />
								</div> --%>
								<div class="span12 field-box">
									<label>用户名称:</label>
			                   ...
			                   <form:select path="users.userId" items="${userList}" itemLabel="userName" itemValue="userId" style="height:30px" ></form:select>
								...
								<div class="span12 field-box">
									<label>企业名称:</label>
									<form:input class="span6" placeholder="企业名称" path="eName" id="eName"/>
								...
								<div class="span12 field-box">
									<label>注册资金:</label>
									<form:input class="span6" placeholder="注册资金" path="registerFund" id="registerFund"/>
								</div>
								<div class="span12 field-box">
									<label>公司地址:</label>
									<form:input class="span6" placeholder="公司地址" path="eAddress" id="eAddress"/>
									<span id="addressError" style="color:red"></span>
									<span id="addressSuccess" style="color:green" ></span>
								</div>
								<div class="span12 field-box">
									<label>上年度中标数:</label>
									<form:input class="span6" placeholder="上年度中标数" path="bidNum" id="bidNum"/>
								</div>
								<div class="span12 field-box">
									<label>成立时间:</label> 
									<form:input type="text" id="dt" placeholder="点击选择日期" path="createTime" />
										<div id="dd"></div>
								</div>
								<div class="span12 field-box">
									<label></label> 
						 ...
						 <div class="span11 field-box actions">
							<input type="submit" value="完成" class="btn-glow primary" />
							<span>或</span> <input type="reset" value="重置" class="reset" />
							<span>或</span> <a href="enterpriseList">返回</a>
						</div>
						</form:form>
						</div>
					</div>
